...
async function runMigrations() {
  ...
  try {
    ...
    // Migration 15.5: User/Commit Systematic Data Manager Schema
    const migration15_5 = 'add-user-commit-data-manager-schema';
    const exists15_5 = await client.query('SELECT name FROM migrations WHERE name = $1', [migration15_5]);
    
    if (exists15_5.rows.length === 0) {
      console.log('🔄 Creating user/commit systematic data management schema...');
      
      // Create users_master table with clear data ownership
      await client.query(`
      ...
      // Create commits_master table with full data
      await client.query(`
        CREATE TABLE IF NOT EXISTS commits_master (
          ...
          -- Analysis
          commit_quality_score DECIMAL(3,2),
          commit_tags TEXT[],
      ...
      // Create user activity snapshots for analytics
      await client.query(`
      ...
      await client.query('CREATE INDEX IF NOT EXISTS idx_users_master_farcaster_fid ON users_master(farcaster_fid)');
      await client.query('CREATE INDEX IF NOT EXISTS idx_users_master_github_username ON users_master(github_username)');
      await client.query('CREATE INDEX IF NOT EXISTS idx_users_master_wallet_address ON users_master(wallet_address)');
      await client.query('CREATE INDEX IF NOT EXISTS idx_users_master_membership_status ON users_master(membership_status)');
      await client.query('CREATE INDEX IF NOT EXISTS idx_users_master_active ON users_master(is_active, total_commits DESC)');

      await client.query('CREATE INDEX IF NOT EXISTS idx_commits_master_hash ON commits_master(commit_hash)');
      await client.query('CREATE INDEX IF NOT EXISTS idx_commits_master_user_id ON commits_master(user_id)');
      await client.query('CREATE INDEX IF NOT EXISTS idx_commits_master_repository ON commits_master(repository_name)');
      await client.query('CREATE INDEX IF NOT EXISTS idx_commits_master_timestamp ON commits_master(commit_timestamp DESC)');
      await client.query('CREATE INDEX IF NOT EXISTS idx_commits_master_processed ON commits_master(processed_at DESC)');
      await client.query('CREATE INDEX IF NOT EXISTS idx_commits_master_reward_status ON commits_master(reward_status)');
      ...
      await client.query('INSERT INTO migrations (name) VALUES ($1)', [migration15_5]);
      console.log('✅ Migration: Created user/commit systematic data management schema');
      console.log('   - users_master: Systematic user data with computed stats');
      console.log('   - commits_master: Full commit data with processing status');
      console.log('   - user_activity_snapshots: Daily analytics snapshots');
    }
